    ...
    try {
      ...
      if (basket === null) {
        ...
        if (obj['basket'].some(item => item.id === product.id)) {
          var sameItemIndex = obj['basket'].findIndex(
        ...
        <View style={tw`flex-1`}>
          <View style={tw`flex-1 relative -mb-3`}>
            <View style={tw`w-full h-72 relative`}>
              ...
              />
              <View
                style={tw`w-14 h-14 rounded-full bg-white shadow-2xl absolute left-4 top-10 justify-center`}>
            ...
            <Animated.View
              style={[
                tw`${isBigScroll ? 'h-full' : 'flex-1'}`,
                {transform: [{translateY: translateAnim}]},
              ]}>
              <ScrollView
                style={[
                  tw`bg-white flex-1 ${
                    isBigScroll && 'absolute left-0 bottom-6 h-5/6'
                ...
                <View style={tw`pb-10`}>
                  <View style={tw`my-4`}>
                    <ThemeText weight={700} style={tw`text-2xl text-black`}>
                      {product.title}
                    </ThemeText>
                  </View>
          ...
          <View
            style={tw` bg-white border-t border-gray-200 h-24 flex-row items-center justify-between p-6`}>
            <View style={tw`flex-row items-center`}>
              <ThemeText weight={800} style={tw`text-2xl text-gray-800`}>
                ${product.price}
              </ThemeText>
              <ThemeText weight={600} style={tw`text-gray-800`}></ThemeText>
            </View>
            <View style={tw`flex-row items-center`}>
              <View style={tw`flex-row bg-gray-200 rounded-full`}>
                <TouchableOpacity
                  style={tw`p-2 w-7 items-center`}
                  onPress={() => amountHandler('lower')}>
                  <Text style={tw`font-bold`}>-</Text>
                </TouchableOpacity>
                <View style={tw`p-2 bg-gray-300 w-7 items-center`}>
                  <Text style={tw`font-bold`}>{amount}</Text>
                </View>
                <TouchableOpacity
                  style={tw`p-2 w-7 items-center`}
                  onPress={() => amountHandler('higher')}>
                  <Text style={tw`font-bold`}>+</Text>
                </TouchableOpacity>
              </View>
              <Pressable
                ...
                style={tw`bg-red-600 items-center px-4 flex-row h-14 rounded-full ml-4`}>
                <ThemeText weight={600} style={tw`text-white mt-1`}>
                  Buy Now
                </ThemeText>
              </Pressable>
            </View>
