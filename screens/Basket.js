...
const Basket = ({navigation}) => {
  ...
  const removeItemFromBasket = async () => {
    ...
    // store güncelleme
    const newBasket = await basket.filter(
      item => item['product'].id !== selectedItem,
    );
  ...
  const updateAmount = async updatetype => {
    ...
    // delete old item
    let removefromBasket = obj['basket'].filter(
    ...
    await AsyncStorage.setItem('basket', removefromBasket);
    // store güncelleme
    // store select item
    const selectItemFromStore = await basket.filter(
      item => item['product'].id === selectedItem,
    );
    // store decrease amount
    let updatedItemFromStore;
    if (updatetype === 'DECREASE') {
      updatedItemFromStore = {
        ...selectItemFromStore[0].product,
        amount: selectItemFromStore[0].product.amount - 1,
      };
    } else if (updatetype === 'INCREASE') {
      updatedItemFromStore = {
        ...selectItemFromStore[0].product,
    ...
    const deleteOldItemFromStore = await basket.filter(
      item => item['product'].id !== selectedItem,
    );
      ...
      <View>
        <Animated.View
          ...
          <Pressable
            ...
            <Icon type="ionicon" name="remove" size={25} color="white" />
          </Pressable>
          <Pressable
            android_ripple={{color: '#dddddd', borderless: false, radius: 70}}
            style={tw`bg-blue-600 w-24 items-center justify-center`}
            onPress={() => updateAmount('INCREASE')}>
            <Icon type="ionicon" name="add" size={25} color="white" />
          </Pressable>
          <Pressable
            android_ripple={{color: '#dddddd', borderless: false, radius: 70}}
            style={tw`bg-black w-24 items-center justify-center`}
            onPress={() =>
              navigation.navigate('ProductScreen', {id: selectedItem})
            }>
            <Icon type="ionicon" name="search" size={25} color="white" />
          </Pressable>
        </Animated.View>
      </View>
    );
    ...
    <>
      ...
      <Wrapper header={false}>
        {!loading && basket.length > 0 && (
          <View
            style={tw`py-2 px-4 bg-black flex-row justify-between items-center`}>
            <View style={tw`h-24 flex-row items-center`}>
              <Text style={tw`font-bold text-white text-xs mr-2`}>
                Total Price:
              </Text>
              <View style={tw`flex-row`}>
                <Text style={tw`font-bold text-white text-lg mt-1`}>
                  ${totalprice}
                  <Text style={tw`text-sm`}>.00</Text>
                </Text>
              </View>
            </View>
            <View style={tw`bg-red-600 rounded-xl overflow-hidden`}>
              <Pressable
                android_ripple={{
                  color: '#dddddd',
                  borderless: false,
            ...
            basket.length > 0 ? (
              basket.map((item, index) => (
                <Swipeable
                  ...
                  onSwipeableOpen={() => setselectedItem(item.product.id)}>
                  <View
                    style={tw`flex-row p-4 bg-white border-b border-gray-100 justify-between items-center`}>
                    <View style={tw`flex-row items-center`}>
                      <View
                        style={tw`mr-2 w-6 h-6 bg-blue-500 rounded-full items-center justify-center`}>
                    ...
                    <Text style={tw`font-bold`}>${item.product.price}</Text>
                  </View>
                </Swipeable>
              ))
            ) : (
              <View
                style={[
                  tw`flex-row p-4 bg-white border-b border-gray-100 justify-center`,
                  styles.centerItems,
                ]}>
                <Text style={styles.text}>There is no any item</Text>
              </View>
            )
          ) : (
